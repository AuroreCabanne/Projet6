...
// Récupère le jeton d'autorisation stocké dans le local storage
let token = localStorage.getItem("token");

// Fonction pour supprimer un projet
function deleteProject(id) {
  // Envoie une requête DELETE pour supprimer le projet correspondant à l'ID spécifié
  ...
  const card = `
    ...
    <img src="${work?.imageUrl} "crossOrigin="anonymous">
      <figcaption>${work?.title}</figcaption>
          ...
          `;
  // Ajoute chaque élément figure à la galerie
...
btn_tous.addEventListener("click", tout); // Ajout d'un événement click sur le bouton avec l'ID "btn_tous" qui appelle la fonction tout
...
// Fonction pour afficher toutes les photos dans le modal
...
function afficheModel() {
  fetch("http://localhost:5678/api/works").then((res) => {
    if (res.ok) {
      res.json().then((data) => {
        // Efface le HTML à l'intérieur de la modale
        document.getElementById("model_gallery").innerHTML = "";
        // Boucle à travers toutes les photos dans le tableau de données pour les afficher dans la modale
        for (let i = 0; i <= data.length - 1; i++) {
          photos(data[i]);
        }
      });
